...
_MEMORY_MAX_BYTES = 16 * 1024 * 1024
...
def _process_private_bytes(process_id: int) -> int | None:
    kernel32, psapi = _private_memory_apis()
        ...
        self._samples.append(
            {
                ...
                "missing_process_ids": list(missing_process_ids),
                "phase": phase,
...
def _summarize(
    ...
    *,
    ...
    python_memory_value = evidence.get("python_queue_memory")
    python_memory = (
        python_memory_value if type(python_memory_value) is dict else {}
    )
    ...
    memory_evidence_valid = bool(
        ...
        and memory_peak <= _MEMORY_MAX_BYTES
    ...
    return {
        "schema_version": 1,
        "gate": "SH-G-8 / BR-G-42 bridge event envelope",
        "passed": passed,
        ...
        "memory": {
            ...
            "python_queue_diagnostic": python_memory,
            "python_queue_shutdown_error": evidence.get(
                "sampler_shutdown_error"
            ),
            "ceiling_bytes": _MEMORY_MAX_BYTES,
        },
        "raw_samples": samples,
...
def _failure_result(
    ...
    *,
    ...
    return {
        ...
        "memory": {"job_private_memory": job_memory},
