...
- MODERATE - FIXED (2026-08-27). Source primitive admission gaps. Core path,
  ...
  source contracts. Fixed with exact types, source-derived ceilings,
  preallocation checks, whole-diagnostic omission, hostile workflow/recorder
  revalidation, the exact predecode settings wall, and complete typed workflow
  walks before JSON projection. Opaque codecs charge repeated wire occurrences,
  reject oversized exact-byte envelopes before parsing, and verify final
  canonical length. Their theoretical schema maxima remain large; later task
  custody must reserve actual charge, and a smaller wire maximum requires
  versioned duplication removal.
