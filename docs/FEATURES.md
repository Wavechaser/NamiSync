...
- **Live Progress**. Executor and verifier events now reach the browser as exact core event v4 snapshots with required phase self-description, paired item identity/type, and an opaque attempt id plus paired attempt-local byte counters when a meaningful stream is active. `SessionEventView` exposes the nested core event version explicitly while the containing bridge remains v1. The type selects the opaque id's row namespace, so post-copy verification keyed by an executor operation remains `operation`; `current_path` stays display-only. Retry or reconstructed resume mints a new attempt id only when the byte pipeline reruns. A stream that exceeds its admitted item total keeps active item/attempt identity but becomes byte-indeterminate. Executor aggregate bytes remain bounded by reviewed content and hold retry high-water; verifier aggregate bytes expand to count monotonic physical-read work across resume. Slice 5 stable-row projection, follow behavior, and rendering remain unrealized, as does scan-specific per-item telemetry.
