...
- **Secured Desktop Host**. The installed wheel now opens only through the pinned Edge Chromium/WebView2 stack, binds bridge authority to the committed loopback origin, blocks external navigation and popups, validates the coordinated database pair before window admission, owns one fixed production instance, and closes through bounded retryable service teardown. Native load does not admit ordinary commands: on startup and same-origin reload, the fixed packaged shell must acknowledge receiver/DOM installation and accept that document generation's initial appearance publication within five seconds before the desktop opens. Close presentation binds the current loaded document before asynchronous work and cannot change shutdown truth on a DOM failure. The packaged document is trusted code while every value it handles remains untrusted data; `DEFENSE.md` owns that trusted-base decision and its reopen trigger. Enforced refusal of an elevated host remains unrealized beta-release work.
