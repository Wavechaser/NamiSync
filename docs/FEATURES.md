...
- **Progress and Cancellation**. Execution reports monotonic aggregate bytes and active-operation identity; byte-producing work additionally reports attempt-local progress. The attempt counter starts at byte-pipeline entry and resets only when a retry or resumed run re-enters that pipeline. Retained post-copy continuations do not reset it. Aggregate bytes hold their high-water until rerun work catches up, including across a serialized pause/resume continuation. Executor pause force-publishes the latest nominal item/attempt state over the frozen last-emitted aggregate/path without falsely settling it; its live aggregate high-water remains continuation state for resume. Cancellation force-publishes an item-inactive terminal snapshot after reliable outcomes while likewise freezing the last emitted legacy totals/path. Execution checks pause/cancellation between operations, at adaptive copy-chunk admission, while a bounded pipeline is backpressured, and around retry backoff. Cancellation remains immediate: a retained pre-publish UPDATE backup is reported without deletion, a published-but-unfinished byte operation fails with `canceled-after-publish`, and a durable or ambiguous non-byte attempt fails with `canceled-after-mutation`; both mutation cases carry explicit state detail, degraded recording, and no false success evidence. When byte-continuation state and a readonly/non-byte marker coexist, cancellation settles both: confirmed publication remains authoritative, while a byte-probe failure cannot mask changed or unverified marker truth.
...
- **Live Progress**. Executor and verifier events now reach the browser with paired item identity/type and paired attempt-local byte counters when a meaningful stream is active. The type selects the opaque id's row namespace, so post-copy verification keyed by an executor operation remains `operation`; `current_path` stays display-only. A stream that exceeds its admitted item total keeps active identity but becomes byte-indeterminate. Executor aggregate bytes remain bounded by reviewed content and hold retry high-water; verifier aggregate bytes expand to count monotonic physical-read work across resume. Slice 5 stable-row projection, follow behavior, and rendering remain unrealized, as does scan-specific per-item telemetry.
