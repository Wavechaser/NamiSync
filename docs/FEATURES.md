...
- **Accept and Re-Baseline**. A file correctly reported as modified can be explicitly re-baselined, accepting its current content as new evidence through the same conditional-recording path instead of remaining reported modified forever. Rebaseline asks for explicit confirmation; baseline and verify do not.
...
- **Typed Recording Attribution** *(accepted Stage 6 target; unrealized)*. Execution review shows filesystem, recording, integrity, and audit truth independently for each item and reports bounded task-level recording problems without letting diagnostic text control behavior. Once an item receipt is committed, later flush, finish, close, audit, or reconciliation trouble cannot rewrite its outcome or hide its durable evidence. Omitted producer detail and presentation-only omission remain visibly separate. Exact reasons, bounds, and result shapes live in `M1_BRIDGE.md`.
...
- **Live Progress**. Execution and verification show stable active-item, phase, retry-attempt, and monotonic progress without presenting telemetry as settlement. Recording problems and large quantities remain exact when the coordinated Stage 6 cutover lands. `M1_BRIDGE.md` owns the transport contract.
