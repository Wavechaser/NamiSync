...
- **Destructive Durability Boundary**. UPDATE and DELETE flush all previously earned recorder evidence before their final destructive guard, so writer contention cannot reopen a guard-to-mutation window. A failure after COPY, UPDATE, or MOVE_UPDATE has published probes durable target/backup/old-path state before cleanup and reports recording as degraded without inventing evidence, including when the publish primitive committed and then raised.
...
- **M1 Evidence Reset Boundary**. Ledger v3/history v4 require immutable final-contract markers. Ledger v1-v2, every history v1-v3 file, and current-version files missing or mismatching those markers are refused read-only and tell the user to close NamiSync and manually recreate both local databases together; normal startup never deletes data. Settings and UI state survive.
- **M1 Windowed History Contract**. History v4 stores an append-only reliable-event journal, dense typed item projections, provisional run watermarks/counts, and bounded terminal phase summaries. The reset boundary is deliberate: the prior history schema cannot reconstruct state/phase events that it never retained.
...
- **Audit Delivery Guarantee**. History subscribes at session admission on the reliable event plane under one clear contract: every audit event is delivered within the timeout, or the session result says `audit=DEGRADED` — nothing is silently lost behind a result claiming OK. Producer offer, window flush, and terminal finalization have independent bounds; failure stops later audit delivery without changing filesystem or ledger truth. Durable finalization success is latched before exact-once observer cleanup, so cleanup failure cannot rewrite the committed/live terminal axis.
- **Bounded Incremental History**. The observer retains at most 256 reliable events and 1 MiB of canonical event JSON, rejects any single event over 1 MiB, and commits at the count/byte bound, one second from the first event in the window, pause, clean close, or finalization. Continuous events never postpone the original age deadline. A crash loses at most the uncommitted window; earlier windows, counts, and committed sequence remain queryable as an incomplete run.
- **Typed Run Details**. History v4 retains ordered reliable envelopes and dense typed result-item projections plus bounded phase summaries. Every item carries explicit phase and item-type tags, so sync and integrity outcomes coexist without duck typing or parallel domain lists.
