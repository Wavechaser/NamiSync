...
- **M0 Scanner Implemented**. Native walking and selected-path observation produce deterministic typed snapshots with exact owned-artifact ignores, conservative capabilities, cooperative cancellation, placeholder/reparse blocking, collision warnings, and explicit completeness. On stable-identity volumes, the native walk recovers an entry identity with a second metadata-only stat when Windows directory enumeration omits it, preserving correspondence-qualified moves without inventing evidence.
...
- **Guarded Deletion**. Internal mirror deletes and empty-directory cleanup validate type and emptiness before removal. Cleanup of a directory emptied by its successful child operations requires the same stable identity and immutable metadata while tolerating only self-induced mtime/link-count churn; identity-less cleanup remains conservative and refuses.
...
- **M0 Workflow And CLI Implemented**. `nami-sync sync` and `python -m namisync sync` now execute the real two-session scanner → planner → preflight → explicit commitment → fresh-preflight → executor/recorder pipeline through dispatcher custody. `nami-sync history` browses the independent audit store; real entry points, no-op history, stale-plan refusal, declined-review non-mutation, database isolation, and import boundaries have integration coverage.
