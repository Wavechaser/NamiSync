...
- **Commit-to-Execute**. Execution happens only for a plan the user has reviewed and explicitly committed; the commitment binds the plan, reviewed selection, and approved task options so none can change under approval. The accepted Stage 6 flow includes the Setup choice for linked verification in that commitment rather than asking again at execution. A committed plan runs immediately when its volumes are free or queues in commit order; an uncommitted plan remains reviewable and never expires into execution. Every execution still preflights first, with material drift refused back to review rather than silently re-planned.
...
- **Plan Review View** *(accepted Stage 6 target; unrealized)*. Plan review combines frozen locations and review-time facts with the current selection, required bytes, risks, notices, search, filters, facets, and rollups. Selection changes do not rewrite the original review result or notices. Execute commits the current nonempty selection and then performs fresh preflight; a new refusal belongs to that execution attempt while the reviewed plan remains visible.
