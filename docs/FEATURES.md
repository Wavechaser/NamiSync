...
- **Exact Large Values** *(accepted Stage 6 target; unrealized)*. File sizes, byte totals, and filesystem timestamps remain exact through persistence and desktop display; unsupported or overflowing observations are refused instead of clamped, rounded, or reinterpreted as capacity. `DEFENSE.md` §1.3 and `M1_BRIDGE.md` own the exact domain and mechanics.
