...
- **Plan Filters**. Plan review filters by operation and blocked/unsupported state with complete-plan category counts. All clears category filters; Copy, Move, Update and Trash remain visible at zero, while less common categories appear when present. Search stays editable through a slow view refresh and the newest queued query runs afterward. Search, filters, and collapse never change selection, and informational notices remain visible context rather than operations. `PRESENTATION.md` owns filtering behavior; exact future facet DTOs are reopened.
...
- **Server-Owned Selection**. Plan selection belongs to the complete server-side operation tree. The synthetic Plan root stays internal; its row is omitted, leaving the header checkbox as the sole whole-view bulk control. It targets selectable operations matching the current search and filters across the complete plan; a folder control targets matching descendants. Neither is limited by the viewport or collapsed state. Search/filter navigation alone never selects or deselects, and Execute uses the complete current selection, including hidden operations. An operation group retains only its direct members, not filesystem-folder scope. The desktop previews and batches changes but cannot invent selection authority; workflow dependency and safety rules still apply.
