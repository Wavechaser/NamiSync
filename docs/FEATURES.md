...
- **Metadata Preservation Scope**. Copies and updates preserve modification time and standard attributes (readonly, hidden, system) by default; alternate data streams and creation time are preserved where supported, and ACL/owner preservation is an explicit, off-by-default policy flag. Observed and intended metadata travel as a typed snapshot on scan records and plan operations — including a per-file stream manifest when ADS preservation is requested — and the readonly attribute is applied only after publish. Alternate data streams are user data: a requested stream that fails to copy on a capable target fails the operation, while a target that cannot support ADS at all surfaces as a reviewable degradation at plan time, before commitment.
