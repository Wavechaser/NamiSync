...
- **Audit Delivery Guarantee**. History subscribes at session admission on the reliable event plane under one clear contract: every audit event is delivered within the timeout, or the session result says `audit=DEGRADED` — nothing is ever silently lost behind a result claiming OK. When its bounded buffer fills, the producer briefly waits at a checkpoint boundary rather than discarding audit events, with the wait capped by a generous timeout — a stalled or failed history writer degrades the session's audit status loudly and blocking stops, rather than holding filesystem work hostage. History finalizes each run through one atomic ownership decision before the immutable terminal event is released: caller ownership at the production cutoff makes both live and late-retained audit truth degraded, while pump ownership waits for the real commit outcome. Production derives its six-second cutoff from the five-second history-writer retry — shorter than the generic writer bound, because exhausting it costs one audit row rather than filesystem or integrity truth — and gives shutdown twelve seconds to cover both sequential bounds plus margin. The producer's bounded wait uses a separate offer timeout, so a long finalization cutoff never stalls live progress. A history write failure is surfaced loudly on the session result but never fails or falsifies filesystem work, and a process crash loses at most the bounded in-flight buffer — the same never-wrong-only-behind posture as the ledger.
