...
- **External-Writer Boundary**. Volume locks arbitrate NamiSync processes only. Operations enforce their preconditions with atomic conditional primitives where Windows provides them — non-replacing renames where absence is expected, `CREATE_NEW` temp creation, emptiness-enforcing directory deletion — and the sole non-conditional window, update's displace-then-replace pair, is a documented microsecond residual: external processes writing into a target root mid-execution are outside the safety contract, and the bounded worst case never corrupts NamiSync's evidence. This is the platform's floor, not a NamiSync shortcut — Explorer's own replace-via-Recycle-Bin flow composes the same displace-then-place steps with the same window, and Windows has offered no supported transactional filesystem primitive since TxF's deprecation.
