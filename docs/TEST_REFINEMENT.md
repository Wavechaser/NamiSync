...
## ST-4 outcome (2026-09-07)

ST-4 replaces two identical light/dark/automatic-theme setup blocks with one narrow helper in `test_design_tokens.py`. The eleven Appendix D functions remain independent detector boundaries; `test_motion.py` remains unchanged because its existing presentation is already compact. All semantic mappings, theme/media scopes, selector-specific negative sets, control states, non-color cues, stylesheet order, dialog exit and virtual-row lifecycle checks remain independent. The four excluded provenance/raw-color/icon functions retain exact raw source and AST identity.
